...
if train_set:
    ...
    NEW_csv = 'ProstateX-Images-Train-NEW.csv'
    ktrans_csv = 'C:\\Users\\haoli\\Downloads\\ProstateX_Data\\ProstateX-TrainingLesionInformationv2\\ProstateX-Images-Ktrans-Train.csv'
    # images_csv = 'C:\\Users\\User\\Mis Documentos\\Mine\\Trabajo\\Uni\\RU\\2ndS-ISMI\\Project\\Data\\Training' \
    #                   '\\ProstateX-TrainingLesi0onInformationv2\\ProstateX-Images-Train.csv'
            ...
            with open(findings_csv, 'r') as findings:
                ...
                for images_row in reader_images:
                    for findings_row in all_findings_rows:
                        if images_row[3].strip() == findings_row[
                            2].strip():  # if the positions match, these rows belong to the same lesion
